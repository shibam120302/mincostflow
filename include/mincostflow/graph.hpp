...
{   
    ...
    class digraph_types
    {
        ...
        // typedef pos_type node_pos_t;
        // typedef pos_type arc_pos_t;
    ...
    */
    {
        ...
        std::vector<bool> is_dual_arc_flag;
        ...
        bool is_valid_arc(arc_pos_t arc)const
        {
            return my_arcs.is_valid(arc);
        }
        bool is_valid_node(node_pos_t node)const
        {
            return my_nodes.is_valid(node);
        }
        bool has_id_node(node_pos_t node)const
        {
            return nodes_ids_flag.at(node);
        }
        bool is_dual_arc(arc_pos_t arc)const
        {
            return is_dual_arc_flag.at(arc);
        }
        bool has_id_arc(arc_pos_t arc)const
        {
        ...
        void erase_arc(arc_pos_t arc)
        {
            ...
            is_dual_arc_flag.resize(my_arcs.capacity());
        }
        void erase_node(node_pos_t node)
        {
        ...
        {
            if(!is_valid_arc(arc1) || !is_valid_arc(arc2))
                throw std::runtime_error("digraph::set_dual invalid arcs");
            
            is_dual_arc_flag.at(arc1)=false;
            is_dual_arc_flag.at(arc2)=true;
        ...
        std::pair<arc_pos_t,arc_pos_t> add_arc(node_pos_t n_a, node_pos_t n_b)
        {
        ...
        std::pair<arc_pos_t,arc_pos_t> add_arc(node_id_t a, node_id_t b, arc_id_t id)
        {
            ...
            if(auto arc = get_arc(id); is_valid_arc(arc))
